...
// Import des composants personnalisés et API
...
const NewTicket = () => {
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      // Préparer les données du ticket avec seulement les propriétés de base
      ...
      // Ajouter les propriétés additionnelles si elles existent dans votre type Ticket
      // Décommentez et ajustez selon votre interface Ticket réelle :
      
      // (ticketData as any).category = form.category || 'General';
      // (ticketData as any).assignee = form.assignee === 'Non assigné' ? '' : form.assignee;
      // (ticketData as any).tags = form.tags;
      // (ticketData as any).authorName = 'Utilisateur actuel';
      // (ticketData as any).authorEmail = 'user@example.com';
      ...
      setTimeout(() => {
        if (isDraft) {
          // Si c'est un brouillon, rester sur la page mais reset le formulaire
          // Note: Comme il n'y a pas de statut draft, on traite comme un ticket normal
          setForm({
            title: '',
            description: '',
            priority: 'medium',
          ...
          // Rediriger vers la liste ou les détails du ticket
      ...
      setError(
        err instanceof Error 
          ...
          : 'Une erreur inattendue s\'est produite lors de la création du ticket'
      );
      setSnackbarOpen(true);
    } finally {
      setLoading(false);
    ...
    <Box sx={{ width: '100vw', minHeight: '100vh', bgcolor: 'background.default', overflowX: 'hidden' }}>
      <Container maxWidth="xl" sx={{ py: { xs: 2, sm: 4 }, px: { xs: 2, sm: 3, md: 4 } }}>
        ...
        <HeaderCard
          ...
          addButtonText="Retour aux tickets"
          addButtonIcon={<ArrowBack />}
          onAddClick={() => navigate('/tickets')}
        />
          ...
          <Paper sx={{ p: 3, borderRadius: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <TextField
                ...
                required
                error={!form.title.trim() && error !== null}
                helperText={!form.title.trim() && error !== null ? 'Le titre est obligatoire' : ''}
                sx={{
              ...
              <TextField
                ...
                required
                error={!form.description.trim() && error !== null}
                helperText={!form.description.trim() && error !== null ? 'La description est obligatoire' : ''}
                sx={{
                  '& .MuiOutlinedInput-root': {
                    borderRadius: 2,
                  }
              ...
              <Box sx={{ display: 'flex', gap: 2, pt: 2 }}>
                ...
                >
                  {loading ? 'Sauvegarde...' : 'Réinitialiser le formulaire'}
                </Button>
              </Box>
            </Box>
          </Paper>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {form.assignee && form.assignee !== 'Non assigné' && (
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      Assigné à
                    </Typography>
